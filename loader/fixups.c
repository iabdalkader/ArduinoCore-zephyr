...
int double_tap_check(void) {
	...
	/* Window during which a second physical reset will be detected as a
	 * "double tap" and bounce the chip into BOOTSEL via reset_usb_boot().
	 * Wider than the original 500ms to be friendlier to human reaction time. */
	k_busy_wait(1500000);
